package com.jarvis.githubissues.queue
 ...
 */
class IssueQueueWorker(
    private val queue: IssueQueue,
    private val client: GitHubApiClient,
    private val onIssueCreated: (fingerprint: String, issueNumber: Int, htmlUrl: String) -> Unit = { _, _, _ -> },
            ...
            while (isActive) {
                ...
                when (val r = client.createIssue(front.draft)) {
                    is GitHubApiClient.Result.Success -> {
                        onIssueCreated(front.draft.fingerprint, r.issueNumber, r.htmlUrl)
                        queue.removeFront()
                    }
