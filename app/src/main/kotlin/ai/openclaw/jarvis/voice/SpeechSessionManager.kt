...
import ai.openclaw.jarvis.memory.ConversationMemoryManager
import ai.openclaw.jarvis.vision.ScreenContextCapture
...
class SpeechSessionManager @Inject constructor(
    private val stt: SpeechToTextProxy,
    ...
    private val listeningModeController: ListeningModeController,
    private val listeningCommandDetector: ListeningCommandDetector,
    private val screenContextCapture: ScreenContextCapture,
    ...
    init {
        ...
        // Always start as the enrolled owner — no need to re-identify on every utterance.
        // Identity check only runs when the session is explicitly reset or has expired.
        if (identityManager.hasProfiles()) {
            trustManager.activateOwnerSession()
        }
        ...
        scope.launch {
            client.events.filterIsInstance<GatewayEvent.AssistantReply>().collect { event ->
                ...
                memoryManager.recordAssistant(reply)
    ...
    private suspend fun runSession(trigger: SessionTrigger) {
        ...
        // Priority 0: listening control commands (silence, stop, pause, resume).
        // Must run before any routing or confirmation so they always fire.
        val listeningCmd = listeningCommandDetector.detect(finalText)
        if (listeningCmd != null) {
            addTranscript("user", finalText, "ListeningControl")
            handleListeningCommand(listeningCmd)
            audioRouter.releaseAudioFocus()
            stateMachine.transition(AssistantState.IDLE_LISTENING, "listening command")
            return
        }
    ...
    private suspend fun forwardToOpenClaw(parsed: ParsedIntent, prefs: JarvisSettings) {
        ...
        if (client.isConnected()) {
            // Attach screen context if the user opted in (requires prior MediaProjection grant).
            val imageBase64 = if (prefs.sendScreenContext)
                screenContextCapture.captureBase64Jpeg() else null

            // Prepend rolling conversation history so the AI model has continuity.
            val message = memoryManager.buildContextualMessage(parsed.rawText)
            memoryManager.recordUser(parsed.rawText)
    ...
    private suspend fun handleListeningCommand(cmd: ListeningCommandDetector.Command) {
        when (cmd) {
            ListeningCommandDetector.Command.SILENCE -> {
                tts.stop()
                listeningModeController.setSilent()
            }
            ListeningCommandDetector.Command.STOP_LISTENING -> {
                tts.stop()
                listeningModeController.setStopped()
            }
            ListeningCommandDetector.Command.PAUSE_10_MIN -> {
                listeningModeController.setPaused(10 * 60 * 1000L)
                speakIfEnabled("Pausing for 10 minutes.")
            }
            ListeningCommandDetector.Command.PAUSE_1_HOUR -> {
                listeningModeController.setPaused(60 * 60 * 1000L)
                speakIfEnabled("Pausing for one hour.")
            }
            ListeningCommandDetector.Command.RESUME_LISTENING -> {
                listeningModeController.setActive()
                speakIfEnabled("I'm listening again.")
            }
        }
    }
