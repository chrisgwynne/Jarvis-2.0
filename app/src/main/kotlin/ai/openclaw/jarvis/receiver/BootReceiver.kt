...
class BootReceiver : BroadcastReceiver() {
    ...
    @Inject lateinit var listeningModeStore: ListeningModeStore
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        scope.launch {
            try {
                // Gateway connection service always starts on boot — the WebSocket
                // connection must be persistent regardless of user preferences.
                GatewayConnectionService.start(context)

                val prefs = settings.settings.first()
                if (prefs.alwaysListeningEnabled) {
                    // Don't restart if the user explicitly stopped listening before the reboot.
                    val mode = listeningModeStore.load()
                    if (mode != ListeningMode.Stopped) {
