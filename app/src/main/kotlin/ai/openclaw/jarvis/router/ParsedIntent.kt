...
data class ParsedIntent(
    ...
    val calendarLookaheadDays: Int? = null,  // for CALENDAR_QUERY
)
...
fun ParsedIntent.toRouteDecision(): RouteDecision = RouteDecision(
    chosen     = when (type) {
        ...
        IntentType.SMS_READ,
        IntentType.CONTACT_LOOKUP     -> RouteChoice.ANDROID_LOCAL
    },
...
fun ParsedIntent.toLegacyIntent(): String = when (type) {
    ...
    IntentType.ENROL_VOICE        -> "enrol_voice"
    IntentType.RECORDING_START    -> "recording_start"
    IntentType.RECORDING_STOP     -> "recording_stop"
    IntentType.CALENDAR_QUERY     -> "calendar_query"
    IntentType.SMS_READ           -> "sms_read"
    IntentType.CONTACT_LOOKUP     -> "contact_lookup"
}
