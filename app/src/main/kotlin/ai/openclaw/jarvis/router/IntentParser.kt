...
class IntentParser @Inject constructor() {
    ...
    private val rules: List<Rule> = listOf(
        ...
        Rule(
            ...
            confidence = 0.93f,
            extract = { _, raw -> { copy(timeAction = TimeSubAction.TIMER, durationMinutes = extractMinutes(raw)) } },
        ),
        Rule(
            patterns = listOf(
            ...
            type = IntentType.TIME_ACTION,
            confidence = 0.92f,
            extract = { _, raw -> { copy(timeAction = TimeSubAction.ALARM, durationMinutes = extractMinutes(raw)) } },
        ),
        Rule(
        ...
        Rule(
            ...
            confidence = 0.91f,
            extract = { m, raw ->
                val c = (m?.groupValues?.getOrNull(2)?.trim()
                    ?: raw.replace(Regex("""^(call|phone|ring|dial)\s+""", IC), "").trim()).ifBlank { null }
                ;{ copy(contact = c) }
            },
        ),
        ...
        Rule(
            ...
            confidence = 0.92f,
            extract = { m, raw ->
                val (rawContact, rawBody) = extractSendEntities(raw, "whatsapp|wa")
                ;{ copy(contact = rawContact, messageBody = rawBody, channel = MessageChannel.WHATSAPP) }
            },
        ),
        ...
        Rule(
            ...
            confidence = 0.92f,
            extract = { m, raw ->
                val (rawContact, rawBody) = extractSendEntities(raw, "email|e-mail|send an? email")
                ;{ copy(contact = rawContact, messageBody = rawBody, channel = MessageChannel.EMAIL) }
            },
        ),
        ...
        Rule(
            ...
            confidence = 0.89f,
            extract = { m, raw ->
                val (rawContact, rawBody) = extractSendEntities(raw, "text|sms|send (a |an )?text( message)?|message|tell")
                ;{ copy(contact = rawContact, messageBody = rawBody, channel = MessageChannel.BEST_AVAILABLE) }
            },
        ),
        ...
        Rule(
            patterns = listOf(
                Regex("""\b(what('s| is) on (my )?(calendar|schedule|agenda))\b""", IC),
                Regex("""\b(any (meetings?|events?|appointments?|calls?)\s*(today|tomorrow|this week)?)\b""", IC),
                Regex("""\b(do i have (anything|any meetings?|any events?) (today|tomorrow|scheduled))\b""", IC),
                Regex("""\b(what('s| is) (happening|planned|scheduled) (today|tomorrow)?)\b""", IC),
                Regex("""\b(show (me )?(my )?(calendar|schedule|agenda))\b""", IC),
            ),
            type = IntentType.CALENDAR_QUERY,
            confidence = 0.92f,
            extract = { _, raw ->
                val days = when {
                    Regex("""\btomorrow\b""", IC).containsMatchIn(raw) -> 2
                    Regex("""\bthis week\b""", IC).containsMatchIn(raw) -> 7
                    else -> 1
                }
                ;{ copy(calendarLookaheadDays = days) }
            },
        ),
        ...
        Rule(
            patterns = listOf(
                Regex("""\b(read (my |any )?(messages?|texts?|sms))\b""", IC),
                Regex("""\b(any (new |unread )?(messages?|texts?|sms))\b""", IC),
                Regex("""\b(do i have (any |new )?(messages?|texts?|sms))\b""", IC),
                Regex("""\b(check (my )?(messages?|texts?|inbox))\b""", IC),
                Regex("""\bwhat('s| is) in my (inbox|messages?)\b""", IC),
            ),
            type = IntentType.SMS_READ,
            confidence = 0.91f,
        ),
        ...
        Rule(
            patterns = listOf(
                Regex("""what('s| is)\s+(.{1,30})'?s?\s+(number|phone|contact)\b""", IC),
                Regex("""\b(find|look up|lookup|search for)\s+(.{1,40})\s+(in\s+)?(contacts?|my phone)\b""", IC),
                Regex("""\bwhat\s+(number|phone)\s+(is|does)\s+(.{1,40})\b""", IC),
            ),
            type = IntentType.CONTACT_LOOKUP,
            confidence = 0.91f,
            extract = { m, raw ->
                val name = m?.groupValues?.drop(1)
